<script lang="ts">
  ...
  import { doLogin, authState } from '../lib/auth.svelte';
  ...
  onMount(() => {
    ...
    if (authState.user) navigate(nextPath, { replace: true });
  });
...
<div class="auth-page">
  <form class="auth-card" onsubmit={(e) => { e.preventDefault(); submit(); }}>
    ...
    <button type="submit" disabled={authState.loading}>
      {#if authState.loading}<Icon name="loader-2" size={15} class="spin" /> 登录中…{:else}登录{/if}
    </button>
