#!/usr/bin/env node
/**
 * 补充 01_产品立项池 缺失的基础字段（使用 @file.json 方式）
 */
...
async function main() {
  ...
  for (const field of fieldsToAdd) {
    ...
    // 写入临时 JSON 文件
    const tmpFile = `/tmp/field_${field.name}.json`;
    fs.writeFileSync(tmpFile, JSON.stringify(field));
    
    const cmd = `lark-cli base +field-create --base-token ${BASE_TOKEN} --table-id ${TABLE_ID} --json @${tmpFile}`;
