...
# 局域网内测启动：前后端都监听 0.0.0.0
pnpm dev:lan

# 如果 5173 被局域网/安全策略拦截，可改用 8002
pnpm dev:lan:8002
```

## 局域网访问

本机启动 `pnpm dev:lan` 后，同一 Wi-Fi/局域网的其他电脑访问：

```text
http://<本机局域网IP>:5173
```

例如本机 IP 是 `192.168.1.23`，内测地址就是 `http://192.168.1.23:5173`。前端会通过 Vite 代理把 `/api` 转发到本机后端 `127.0.0.1:3000`，所以同事只需要打开 5173 端口的前端地址。

如果 `5173` 无法被局域网访问，也可以启动 `pnpm dev:lan:8002`，访问：

```text
http://<本机局域网IP>:8002
```

如果同网段仍打不开，优先检查 Windows 防火墙是否放行 Node.js 或端口 `5173`。可在本机 PowerShell 查看监听地址：
...
看到 `0.0.0.0:5173` / `0.0.0.0:3000` 才表示服务已对局域网监听；如果是 `127.0.0.1:5173`，说明启动命令或配置没有走局域网模式。
